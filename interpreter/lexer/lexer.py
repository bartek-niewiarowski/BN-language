...
class Lexer:
    ...
    def _build_identifire_or_keyword(self) -> Optional[Token]:
        ...
        if buffer == '':
            return None
        elif tokenType := self.keywords.get(buffer): # tokenType := self.keywords.get(buffer)
    ...
    def _build_one_line_operator(self) -> Optional[Token]:
        if tokenType := self.one_line_operators.get(self._current_char): # to samo co wyzej
    ...
    def _build_float(self, int_part) -> Optional[Token]:
        self._next_char()
        if not self._current_char.isdecimal():
    ...
    def build_one_or_two_char_token(self, one_char_token: tuple[str, TokenType],
        ...
        for element in two_chars_tokens:
            if self._current_char == element[0]:
                return Token(element[1], None, self._source.get_position())        
